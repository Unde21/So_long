...
/*   Updated: 2025/02/21 04:41:49 by samaouch         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
		data->enemy->sprite[SHIELD], data->enemy->pos_x * 64, data->enemy->pos_y
		* 64);
	if (data->map[data->enemy->pos_y - 1][data->enemy->pos_x] == 'C')
		display_collectible(data, img, data->enemy->pos_x,
			data->enemy->pos_y - 1);
	else
	...
	if (data->map[data->enemy->pos_y][data->enemy->pos_x + 1] == 'C')
		display_collectible(data, img, data->enemy->pos_x + 1,
			data->enemy->pos_y);
	else
...
{
	mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, img->sprite[OBJECT], x
		* 64, y * 64);
}
