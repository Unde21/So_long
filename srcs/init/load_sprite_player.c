...
/*   Updated: 2025/02/19 23:23:07 by samaouch         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	load_img_player(t_data *data, t_player *player)
{
	...
	while (i < 42)
	{
	player->sprite[i] = mlx_xpm_file_to_image(data->mlx_ptr, player->img[i], &data->img->width, &data->img->height);
		if (!player->sprite[i])
			return (ERR_IMG);
		++i;
	}
...
// 		return (ERR_IMG);
// 	player->sprite[FINAL_STARTR_D] = mlx_xpm_file_to_image(data->mlx_ptr,
